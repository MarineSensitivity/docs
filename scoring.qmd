...
See the [Scoring Walkthrough](scoring-walkthrough.qmd) for an animated, step-by-step visualization of this pipeline — from input rasters through MAX merge, ecoregional rescaling, and component averaging.
:::
...
The formula above applies to **most species** (fish, birds, mammals, corals, invertebrates, other), where `er_score` is a **scalar** value per species (1--100) and `v` is the MAX-merged suitability value.

For **sea turtles**, extinction risk is already embedded in the merged model values via the multiplicative merge (see @sec-model-merging, Step 2b). In this case the scoring formula simplifies to:

$$
\text{score}_{c,g} = \sum_{s=1}^{S_g} v_{merged,s,c}
$$ {#eq-cell-score-turtle}

because `er_score = 100` is used as a pass-through multiplier (dividing by 100 cancels it out). The spatially varying extinction risk from the SWOT+DPS raster is already encoded in $v_{merged}$, so cells within an Endangered DPS contribute higher values than cells within a Threatened DPS, even for the same species and suitability level.
...
- **[Scores](https://app.marinesensitivity.org/scores/)**: general sensitivity mapping app showing composite scores by program area with flower plots and treemaps (see [Appendix: mapgl](apps/mapgl.qmd))
- **[Species](https://app.marinesensitivity.org/species/)**: species distribution viewer showing individual and merged models with regulatory status information (see [Appendix: mapsp](apps/mapsp.qmd))
