...
- **Interactive applications** — Scores and Species — render **any published release** from the URL path (e.g. `/v3/scores/`; an older `?ver=v3` deep link still resolves, 301'd to the equivalent path), so a reader can look at the release a report cited rather than only the newest one.
