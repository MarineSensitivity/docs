...
```{r}
#| include: false
# an empty setup chunk is still required for inline (`r ...`) calls in non-html
# output; `g()` and the doc_* helpers arrive via .Rprofile (libs/functions.R,
# libs/versioned.R)
```
...
1. **Species** — by taxonomic category (bird, coral, fish, invertebrate, mammal, reptile/turtle): extinction risk weighted species distribution models across `r doc_stat("valid_species")` valid species (see @sec-data-sources, @sec-extinction-risk)
