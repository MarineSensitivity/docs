...
- **NMFS Distinct Population Segments** (DPS) from the NOAA Fisheries ESA Species Range Geodatabase [@noaafisheries2025dps], which identify sub-populations listed as Endangered under the ESA.
...
The merged model is the derived output of the model merging pipeline (see @sec-model-merging). For each of the `r n_fmt(mst_stats$valid_species)` valid species, cell values are computed as:
